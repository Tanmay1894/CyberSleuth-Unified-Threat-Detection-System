...
def analyze_url(url, source="Manual", session_id=None):
    ...
    # GSB Check (with caching)
    gsb_result = check_gsb(url)

    # ML Check (with real features)
    ...
    risk_score, reasons = calculate_risk_score(url, gsb_result, ml_result, features)

    # Determine final verdict based on risk score
    if gsb_result.get('status') == "MALICIOUS" or risk_score >= RISK_THRESHOLD_MALICIOUS:
